const shuffle = (list) => {
    ...
    let currentIndex = tempList.length, randomIndex;
    ...
    while (currentIndex !== 0) {
        ...
        [tempList[currentIndex], tempList[randomIndex]] = [tempList[randomIndex], tempList[currentIndex]];
    }
...
if (images.length === randomUrls.length) {
    images.forEach((img, index) => {
        img.src = randomUrls[index];
    });
} else {
    console.error('Mismatch in the number of images and URLs');
}
